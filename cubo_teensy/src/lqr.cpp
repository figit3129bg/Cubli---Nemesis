...
// PID state-feedback gains.
constexpr double kK1 = -270.5849;  // theta_b     [rad]   -> N*m
constexpr double kK2 = -21.5554;   // theta_b_dot [rad/s] -> N*m
constexpr double kK3 = -1.0024;    // theta_w_dot [rad/s] -> N*m
...
// ever change.
constexpr double kMaxTorqueNm = 5;
...
int main(int argc, char** argv) {
    ...
    csv << "time_s,theta_b,theta_b_dot,theta_w_dot,"
           "theta_b_hat,theta_b_dot_hat,theta_w_dot_hat,raw_torque,torque\n";
    ...
    while (g_running && port.readLine(line)) {
        ...
        const double torque =
            std::clamp(raw_torque, -kMaxTorqueNm, kMaxTorqueNm);
        prev_torque = torque;
